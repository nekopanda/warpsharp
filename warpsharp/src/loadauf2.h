...
class ClampFrameRange2 : public GenericVideoFilter {
public:
	ClampFrameRange2(PClip clip) : GenericVideoFilter(clip)
...
class AviUtlFrameCache2 : public GenericVideoFilter {
...
public:
	AviUtlFrameCache2(PClip clip, int align)
...
class AviUtlFilterProxyExFunc2 {
public:
...
class AviUtlFilterPluginExFunc2 : public FILTER {
public:
...
class ExFuncCallback2 {
	...
	static void * __cdecl get_ycp_filtering(void *fp, void *editp, int n, void *reserve) {
		...
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_ycp_filtering(n);
	}
	...
	static void * __cdecl get_ycp_filtering_cache(void *fp, void *editp, int n) {
		...
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_ycp_filtering_cache(n);
	}
	...
	static BOOL __cdecl get_pixel_filtered(void *editp, int n, void *pixelp, int *w, int *h) {
		...
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_pixel_filtered(n, pixelp, w, h);
	}
	...
	static BOOL __cdecl get_pixel_source(void *editp, int n, void *pixelp, DWORD format) {
		...
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_pixel_source(n, pixelp, format);
	}
	...
	static BOOL __cdecl get_pixel_filtered_ex(void *editp, int n, void *pixelp, int *w, int *h, DWORD format) {
		...
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_pixel_filtered_ex(n, pixelp, w, h, format);
	}


	static int __cdecl get_audio(void *editp, int n, void *buf) {
		if(DeBug) dout << "get_audio(" << editp << "," << n << "," << buf << ")" << std::endl;
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_audio(n, buf);
	}

	static int __cdecl get_audio_filtered(void *editp, int n, void *buf) {
		if(DeBug) dout << "get_audio_filtered(" << editp << "," << n << "," << buf << ")" << std::endl;
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_audio_filtered(n, buf);
	}
	...
	static BOOL __cdecl get_sys_info(void *editp, SYS_INFO *sip) {
		...
		if(editp)
			return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_sys_info(sip);
		else
			return true;
	}
	...
	static BOOL __cdecl get_frame_status(void *editp, int n, FRAME_STATUS *fsp) {
		...
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_frame_status(n, fsp);
	}
	...
	static BOOL __cdecl set_frame_status(void *editp, int n, FRAME_STATUS *fsp) {
		...
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->set_frame_status(n, fsp);
	}


	static BOOL __cdecl get_file_info(void *editp, FILE_INFO *fip) {
		if(DeBug) dout << "get_file_info(" << editp << "," << fip << ")" << std::endl;
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_file_info(fip);
	}
	...
	static BOOL __cdecl get_source_file_info(void *editp, FILE_INFO *fip, int source_file_id) {
		...
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_source_file_info(fip, source_file_id);
	}
	...
	static BOOL __cdecl get_source_video_number(void *editp, int n, int *source_file_id, int *source_video_number) {
		...
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_source_video_number(n, source_file_id, source_video_number);
	}


	static LPSTR __cdecl get_config_name(void *editp, int n) {
		if(DeBug) dout << "get_config_name(" << editp << "," << n << ")" << std::endl;
		return (LPSTR)static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_config_name(n);
	}

	static BOOL __cdecl get_select_frame(void *editp, int *s, int *e) {
		if(DeBug) dout << "get_select_frame(" << editp << "," << s << "," << e << ")" << std::endl;
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->get_select_frame(s, e);
	}

	static BOOL __cdecl set_select_frame(void *editp, int s, int e) {
		if(DeBug) dout << "set_select_frame(" << editp << "," << s << "," << e << ")" << std::endl;
		return static_cast<AviUtlFilterProxyExFunc2 *>(editp)->set_select_frame(s, e);
	}
	...
	static BOOL __cdecl set_ycp_filtering_cache_size(void *fp, int w, int h, int d, int flag) {
		...
		return static_cast<AviUtlFilterPluginExFunc2 *>(static_cast<FILTER *>(fp))->set_ycp_filtering_cache_size(w, h, d);
	}


	static BOOL __cdecl filter_window_update(void *fp) {
		if(DeBug) dout << "filter_window_update(" << fp << ")" << std::endl;
		return static_cast<AviUtlFilterPluginExFunc2 *>(static_cast<FILTER *>(fp))->filter_window_update();
	}

	static BOOL __cdecl is_filter_window_disp(void *fp) {
		if(DeBug) dout << "is_filter_window_disp(" << fp << ")" << std::endl;
		return static_cast<AviUtlFilterPluginExFunc2 *>(static_cast<FILTER *>(fp))->is_filter_window_disp();
	}

	static BOOL __cdecl is_filter_active(void *fp) {
		if(DeBug) dout << "is_filter_active(" << fp << ")" << std::endl;
		return static_cast<AviUtlFilterPluginExFunc2 *>(static_cast<FILTER *>(fp))->is_filter_active();
	}


	static int __cdecl ini_load_int(void *fp, LPSTR key, int n) {
		if(DeBug) dout << "ini_load_int(" << fp << "," << key << "," << n << ")" << std::endl;
		return static_cast<AviUtlFilterPluginExFunc2 *>(static_cast<FILTER *>(fp))->ini_load_int(key, n);
	}

	static int __cdecl ini_save_int(void *fp, LPSTR key, int n) {
		if(DeBug) dout << "ini_save_int(" << fp << "," << key << "," << n << ")" << std::endl;
		return static_cast<AviUtlFilterPluginExFunc2 *>(static_cast<FILTER *>(fp))->ini_save_int(key, n);
	}

	static BOOL __cdecl ini_load_str(void *fp, LPSTR key, LPSTR str, LPSTR def) {
		if(DeBug) dout << "ini_load_str(" << fp << "," << key << "," << str << "," << def << ")" << std::endl;
		return static_cast<AviUtlFilterPluginExFunc2 *>(static_cast<FILTER *>(fp))->ini_load_str(key, str, def);
	}

	static BOOL __cdecl ini_save_str(void *fp, LPSTR key, LPSTR str) {
		if(DeBug) dout << "ini_save_str(" << fp << "," << key << "," << str << ")" << std::endl;
		return static_cast<AviUtlFilterPluginExFunc2 *>(static_cast<FILTER *>(fp))->ini_save_str(key, str);
	}
...
class AviUtlFilterPlugin2;
typedef Ref<AviUtlFilterPlugin2> AviUtlFilterPluginRef2;

class AviUtlFilterPluginParams2;
typedef Ref<AviUtlFilterPluginParams2> AviUtlFilterPluginParamsRef2;

class AviUtlFilterPluginParams2 : public RefCount<AviUtlFilterPluginParams2> {
public:
	virtual ~AviUtlFilterPluginParams2() {}
	virtual AviUtlFilterPluginRef2 CreatePlugin() = 0;
...
class AviUtlFilterPlugin2
	...
	, protected AviUtlFilterPluginExFunc2
{
	std::string _params;
...
public:
	AviUtlFilterPlugin2(const char *path, const char *name, bool copy, bool debug, int thread)
		...
		, _debug(debug), _cache_w(0), _cache_h(0), _cache_d(0), _thread(thread)
	{
		...
		if(debug) dout << "AviUtlFilterPlugin2(" << path << "," << name << "," << copy << "," << debug << "," << thread << ")" << std::endl;
		...
		exfunc = debug ? ExFuncCallback2<true>::GetExFunc() : ExFuncCallback2<false>::GetExFunc();
		hwnd = HWND_DESKTOP;
	...
	bool SaveStart(int s, int e, AviUtlFilterProxyExFunc2 *editp)
	{ return (func_save_start != NULL) ? !!func_save_start(static_cast<FILTER *>(this), s, e, editp) : true; }

	bool SaveEnd(AviUtlFilterProxyExFunc2 *editp)
	{ return (func_save_end != NULL) ? !!func_save_end(static_cast<FILTER *>(this), editp) : true; }
};

//////////////////////////////////////////////////////////////////////////////

class AviUtlFilterProxy2
	: public GenericVideoFilter
	, protected AviUtlFilterProxyExFunc2
{
	AviUtlFilterPluginRef2 _plugin;
	...
	AviUtlFrameCache2 *_cacheYC, *_cacheEX, *_cacheYUY, *_cacheRGB;
	...
	AviUtlFrameCache2 *_get_pixel_cache(DWORD format) {
		switch(format) {
	...
	void *_get_pixel_ptr(int n, DWORD format) {
		_writable = false;
		AviUtlFrameCache2 *cache = _get_pixel_cache(format);
	...
	AviUtlFilterProxy2(const AviUtlFilterPluginRef2& plugin, AVSValue args, IScriptEnvironment *env)
		: GenericVideoFilter(new ClampFrameRange2(args[0].AsClip()))
		, _plugin(plugin), _writable(true)
	{
		_clipYC = _cacheYC = new AviUtlFrameCache2(child, sizeof(PIXEL_YC));
		_clipEX = _cacheEX = new AviUtlFrameCache2(child, sizeof(PIXEL_YC));
		...
		_clipYUY = _cacheYUY = new AviUtlFrameCache2(_clipYUY, 4);
		_clipRGB = _cacheRGB = new AviUtlFrameCache2(_clipRGB, 4);
	...
	static AVSValue __cdecl Create(AVSValue args, void *user_data, IScriptEnvironment *env) {
		AviUtlFilterPluginParams2 *params = static_cast<AviUtlFilterPluginParams2 *>(user_data);
		AviUtlFilterPluginRef2 plugin(params->CreatePlugin());
		...
		return new AviUtlFilterProxy2(plugin, args, env);
	}

public:
	static void PluginInit(const AviUtlFilterPluginParamsRef2& params, IScriptEnvironment *env)
	...
	PVideoFrame __stdcall GetFrame(int n, IScriptEnvironment *env) {
		...
		VideoInfo vi2 = vi;
		vi2.height = vi.height * 2;
		DWORD offset = vi.width * vi.height * sizeof(PIXEL_YC) / 6;
		PVideoFrame dubleFrame(env->NewVideoFrame(vi2, sizeof(PIXEL_YC)));
		env->BitBlt(dubleFrame->GetWritePtr() + offset, editFrame->GetPitch(), editFrame->GetReadPtr(), editFrame->GetPitch(), vi.RowSize(), vi.height);
		editFrame = dubleFrame;

		if(!editFrame->IsWritable() || editFrame->GetPitch() != vi2.RowSize()) {
			PVideoFrame workFrame(env->NewVideoFrame(vi2, sizeof(PIXEL_YC)));
			env->BitBlt(workFrame->GetWritePtr(), workFrame->GetPitch(),
			editFrame->GetReadPtr(), editFrame->GetPitch(), vi2.RowSize(), vi2.height);
			editFrame = workFrame;
		}

		if(!_tempFrame || !_tempFrame->IsWritable())
			_tempFrame = env->NewVideoFrame(vi2, sizeof(PIXEL_YC));

		PIXEL_YC *edit = (PIXEL_YC *)( editFrame->GetWritePtr() + offset);
		PIXEL_YC *temp = (PIXEL_YC *)(_tempFrame->GetWritePtr() + offset);
		...
		fpi.audio_ch = 0;
		fpi.pixelp = NULL;
		fpi.editp = static_cast<AviUtlFilterProxyExFunc2 *>(this);
		fpi.yc_size = sizeof(PIXEL_YC);
		fpi.line_size = fpi.yc_size * fpi.w;

		//if(_plugin->IsDebug){
		//	dout << "---------- " << fpi.frame << "/" << fpi.frame_n << " ----------" << std::endl;
		//	dout << "fpi.w = " << fpi.w << std::endl;
		//	dout << "fpi.h = " << fpi.w << std::endl;
		//	dout << "fpi.ycp_edit = " << fpi.ycp_edit << std::endl;
		//	dout << "fpi.ycp_temp = " << fpi.ycp_temp << std::endl;
		//	dout << "" << std::endl;
		//}
		...
		PVideoFrame result(env->NewVideoFrame(vi, sizeof(PIXEL_YC)));
		env->BitBlt(result->GetWritePtr(), result->GetPitch(), (BYTE *)fpi.ycp_edit, result->GetPitch(), vi.RowSize(), vi.height);
...
class AviUtlFilterPluginByRef2 : public AviUtlFilterPluginParams2 {
	AviUtlFilterPluginRef2 _plugin;

public:
	AviUtlFilterPluginByRef2(const AviUtlFilterPluginRef2& plugin)
		: _plugin(plugin) {}

	AviUtlFilterPluginRef2 CreatePlugin()
	{ return _plugin; }
...
class AviUtlFilterPluginByCopy2 : public AviUtlFilterPluginParams2 {
	std::string _path, _name, _args, _info;
	bool _debug;
	int _thread;

public:
	AviUtlFilterPluginByCopy2(const AviUtlFilterPluginRef2& plugin)
	...
	AviUtlFilterPluginRef2 CreatePlugin()
	{ return new AviUtlFilterPlugin2(_path.c_str(), _name.c_str(), true, _debug, _thread); }
...
class LoadAviUtlFilterPlugin2 {
	std::map<std::string, AviUtlFilterPluginParamsRef2> _params;

//	static void __cdecl Delete(void *user_data, IScriptEnvironment *env)
//	{ delete static_cast<LoadAviUtlFilterPlugin2 *>(user_data); }

	static AVSValue __cdecl Create(AVSValue args, void *user_data, IScriptEnvironment *env)
	{ return Singleton<LoadAviUtlFilterPlugin2>::GetInstance(env)->AddPlugin(args, env); }
//	{ return static_cast<LoadAviUtlFilterPlugin2 *>(user_data)->AddPlugin(args, env); }
	...
	AVSValue AddPlugin(AVSValue args, IScriptEnvironment *env) {
		...
		AviUtlFilterPluginRef2 plugin(new AviUtlFilterPlugin2(path, name, false, debug, thread));
		...
		AviUtlFilterPluginParamsRef2& params = _params[name];

		if(copy)
			params = new AviUtlFilterPluginByCopy2(plugin);
		else
			params = new AviUtlFilterPluginByRef2(plugin);

		AviUtlFilterProxy2::PluginInit(params, env);
...
public:
	...
	{ return "LoadAviUtlFilterPlugin2"; }

	static void PluginInit(IScriptEnvironment *env) {
		LoadAviUtlFilterPlugin2 *self = new LoadAviUtlFilterPlugin2();
